...
function matchSignature(
  ...
  return {
    ...
    events: wrapLeaf(ctx, cursor, cursor + run, 'signature', signatureProps(run as 3 | 4 | 5)),
  };
}
...
function matchTemplate(
  ...
  const outer_start = pointAt(ctx, cursor);
  const outer_end = pointAt(ctx, close_end);
  const events: WikitextEvent[] = [enterEventFromPoints(node_type, { name }, outer_start, outer_end)];
  ...
  events.push(exitEventFromPoints(node_type, outer_start, outer_end));
...
function appendTemplateArgument(
  ...
  const arg_start = pointAt(ctx, start_offset);
  const arg_end = pointAt(ctx, end_offset);

  events.push(enterEventFromPoints('template-argument', props, arg_start, arg_end));
  appendInlineRange(events, ctx, value_start, end_offset, true);
  events.push(exitEventFromPoints('template-argument', arg_start, arg_end));
}
...
function matchWikilink(
  ...
  const outer_start = pointAt(ctx, cursor);
  const outer_end = pointAt(ctx, close_end);
  const events: WikitextEvent[] = [enterEventFromPoints(node_type, { target }, outer_start, outer_end)];
...
function matchExternalLink(
  ...
  const outer_start = pointAt(ctx, cursor);
  const outer_end_point = pointAt(ctx, outer_end);
  const events: WikitextEvent[] = [enterEventFromPoints('external-link', { url }, outer_start, outer_end_point)];
  ...
  events.push(exitEventFromPoints('external-link', outer_start, outer_end_point));
...
function matchBareUrl(
  ...
  const start_point = pointAt(ctx, url_start);
  const end_point = pointAt(ctx, url_end);
  return {
    start_offset: url_start,
    end_offset: url_end,
    events: [
      enterEventFromPoints('external-link', { url }, start_point, end_point),
      exitEventFromPoints('external-link', start_point, end_point),
    ],
  };
}
...
function matchTagLike(
  ...
  if (tag.tag_name_lower === 'br') {
    return {
      ...
      events: wrapLeaf(ctx, cursor, tag.end_offset, 'break', EMPTY_EVENT_PROPS),
    };
  }
...
function emitText(
  ...
  return textEventFromPoints(
    start_offset,
    end_offset,
    pointAt(ctx, start_offset),
    pointAt(ctx, end_offset),
  );
}
...
function wrapLeaf(
  ...
  const start_point = pointAt(ctx, start_offset);
  const end_point = pointAt(ctx, end_offset);
  return [
    enterEventFromPoints(node_type, props, start_point, end_point),
    exitEventFromPoints(node_type, start_point, end_point),
  ];
}
...
function createWrappedInlineEvents(
  ...
  const start_point = pointAt(ctx, start_offset);
  const end_point = pointAt(ctx, end_offset);
  const events: WikitextEvent[] = [enterEventFromPoints(node_type, props, start_point, end_point)];
  appendInlineRange(events, ctx, content_start, content_end, allow_bare_url);
  events.push(exitEventFromPoints(node_type, start_point, end_point));
  return events;
}
