...
import {
  enterEventFromPoints,
  exitEventFromPoints,
  textEventFromPoints,
} from './event_factory.ts';
...
export function* blockEvents(
  ...
  yield enterEventFromPoints('root', EMPTY_EVENT_PROPS, startPt, startPt);
  ...
  yield exitEventFromPoints('root', endPt, endPt);
}
...
function* parseHeading(
  ...
  yield enterEventFromPoints('heading', headingProps(level), startPt, endPt);
  ...
  yield exitEventFromPoints('heading', startPt, endPt);
}
...
function* parseParagraph(
  ...
  yield enterEventFromPoints('paragraph', EMPTY_EVENT_PROPS, startPt, endPt);
  ...
  yield exitEventFromPoints('paragraph', startPt, endPt);
}
  ...
  while (peek(buf) !== null) {
    ...
    for (let i = openStack.length; i < depth; i++) {
      ...
      if (lvl.list_type === 'list') {
        yield enterEventFromPoints('list', listProps(lvl.ordered), lvlPt, lvlPt);
      } else {
        yield enterEventFromPoints('definition-list', EMPTY_EVENT_PROPS, lvlPt, lvlPt);
      }
    ...
    if (lastLevel.kind === 'definition-term') {
      yield enterEventFromPoints('definition-term', EMPTY_EVENT_PROPS, itemPt, itemPt);
    } else if (lastLevel.kind === 'definition-description') {
      yield enterEventFromPoints('definition-description', EMPTY_EVENT_PROPS, itemPt, itemPt);
    } else {
      yield enterEventFromPoints('list-item', { marker: fullMarker }, itemPt, itemPt);
    }
    ...
    // Close the list item.
    if (lastLevel.kind === 'definition-term') {
      yield exitEventFromPoints('definition-term', itemEndPt, itemEndPt);
    } else if (lastLevel.kind === 'definition-description') {
      yield exitEventFromPoints('definition-description', itemEndPt, itemEndPt);
    } else {
      yield exitEventFromPoints('list-item', itemEndPt, itemEndPt);
    }
...
function* closeLevels(
  ...
  while (stack.length > targetDepth) {
    ...
    if (entry.level.list_type === 'list') {
      yield exitEventFromPoints('list', closePt, closePt);
    } else {
      yield exitEventFromPoints('definition-list', closePt, closePt);
    }
  }
}
...
function* parseTable(
  ...
  yield enterEventFromPoints('table', attributes !== undefined ? { attributes } : EMPTY_EVENT_PROPS, startPt, startPt);
  ...
  while (peek(buf) !== null) {
    ...
    if (t.type === TokenType.TABLE_CLOSE) {
      ...
      advance(buf);
      yield exitEventFromPoints('table', closePt, closePt);
      // Consume trailing newline.
      if (peek(buf) !== null && peek(buf)!.type === TokenType.NEWLINE) {
        advance(buf);
      }
    ...
    if (t.type === TokenType.TABLE_ROW) {
      ...
      yield enterEventFromPoints(
        'table-row',
        rowAttrs !== undefined ? { attributes: rowAttrs } : EMPTY_EVENT_PROPS,
        rowPt,
        rowPt,
      );
      rowOpen = true;
    ...
    if (t.type === TokenType.TABLE_CAPTION) {
      ...
      yield enterEventFromPoints('table-caption', EMPTY_EVENT_PROPS, capPt, capPt);
      ...
      yield exitEventFromPoints('table-caption', capEndPt, capEndPt);
    ...
    if (t.type === TokenType.TABLE_HEADER_CELL) {
      ...
      if (!rowOpen) {
        ...
        // an explicit `|-` line.
        const rowPt = pointAt(buf.tracker, t.start);
        yield enterEventFromPoints('table-row', EMPTY_EVENT_PROPS, rowPt, rowPt);
        rowOpen = true;
      }
      advance(buf);
  ...
  yield exitEventFromPoints('table', endPt, endPt);
}
...
function* parseTableCells(
  ...
  while (true) {
    ...
    yield enterEventFromPoints('table-cell', tableCellProps(header), cellPt, cellPt);
    ...
    const cellEndPt = contentSpans.length > 0
      ...
      : cellPt;
    yield exitEventFromPoints('table-cell', cellEndPt, cellEndPt);
...
function* closeCell(buf: TokenBuffer): Generator<WikitextEvent> {
  const pt = peek(buf)
    ? pointAt(buf.tracker, peek(buf)!.start)
    : point(buf.tracker.line, 1, buf.tracker.line_offset);
  yield exitEventFromPoints('table-cell', pt, pt);
}
...
function* closeRow(buf: TokenBuffer): Generator<WikitextEvent> {
  const pt = peek(buf)
    ? pointAt(buf.tracker, peek(buf)!.start)
    : point(buf.tracker.line, 1, buf.tracker.line_offset);
  yield exitEventFromPoints('table-row', pt, pt);
}
...
function* emitTextSpans(
  ...
  for (const span of spans) {
    ...
    yield textEventFromPoints(span.start, span.end, eventStart, eventEnd);
  }
}
...
function* parseThematicBreak(
  ...
  yield enterEventFromPoints('thematic-break', EMPTY_EVENT_PROPS, startPt, endPt);
  yield exitEventFromPoints('thematic-break', startPt, endPt);
}
...
function* parsePreformatted(
  ...
  yield enterEventFromPoints('preformatted', EMPTY_EVENT_PROPS, startPt, startPt);
