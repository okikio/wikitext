...
export function* blockEvents(
  ...
  yield enterEventFromPoints('root', {}, startPt, startPt);
...
function* parseHeading(
  ...
  yield enterEventFromPoints('heading', { level }, startPt, endPt);
...
function* parseParagraph(
  ...
  yield enterEventFromPoints('paragraph', {}, startPt, endPt);
  ...
  while (peek(buf) !== null) {
    ...
    for (let i = openStack.length; i < depth; i++) {
      ...
      if (lvl.list_type === 'list') {
        yield enterEventFromPoints('list', { ordered: lvl.ordered }, lvlPt, lvlPt);
      } else {
        yield enterEventFromPoints('definition-list', {}, lvlPt, lvlPt);
      }
    ...
    if (lastLevel.kind === 'definition-term') {
      yield enterEventFromPoints('definition-term', {}, itemPt, itemPt);
    } else if (lastLevel.kind === 'definition-description') {
      yield enterEventFromPoints('definition-description', {}, itemPt, itemPt);
    } else {
...
function* parseTable(
  ...
  yield enterEventFromPoints('table', attributes !== undefined ? { attributes } : {}, startPt, startPt);
  ...
  while (peek(buf) !== null) {
    ...
    if (t.type === TokenType.TABLE_CAPTION) {
      ...
      yield enterEventFromPoints('table-caption', {}, capPt, capPt);
    ...
    if (t.type === TokenType.TABLE_HEADER_CELL) {
      ...
      if (!rowOpen) {
        ...
        // an explicit `|-` line.
        const rowPt = pointAt(buf.tracker, t.start);
        yield enterEventFromPoints('table-row', {}, rowPt, rowPt);
        rowOpen = true;
      }
      advance(buf);
...
function* parseTableCells(
  ...
  while (true) {
    ...
    yield enterEventFromPoints('table-cell', { header }, cellPt, cellPt);
...
function* parseThematicBreak(
  ...
  yield enterEventFromPoints('thematic-break', {}, startPt, endPt);
