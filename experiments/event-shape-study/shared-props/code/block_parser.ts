...
export function* blockEvents(
  ...
  yield enterEvent('root', EMPTY_EVENT_PROPS, zeroPos(startPt));
...
function* parseHeading(
  ...
  yield enterEvent('heading', headingProps(level), headingPos);
...
function* parseParagraph(
  ...
  yield enterEvent('paragraph', EMPTY_EVENT_PROPS, paraPos);
  ...
  while (peek(buf) !== null) {
    ...
    for (let i = openStack.length; i < depth; i++) {
      ...
      if (lvl.list_type === 'list') {
        yield enterEvent('list', listProps(lvl.ordered), zeroPos(lvlPt));
      } else {
        yield enterEvent('definition-list', EMPTY_EVENT_PROPS, zeroPos(lvlPt));
      }
    ...
    if (lastLevel.kind === 'definition-term') {
      yield enterEvent('definition-term', EMPTY_EVENT_PROPS, zeroPos(itemPt));
    } else if (lastLevel.kind === 'definition-description') {
      yield enterEvent('definition-description', EMPTY_EVENT_PROPS, zeroPos(itemPt));
    } else {
...
function* closeLevels(
  ...
  while (stack.length > targetDepth) {
    ...
    if (entry.level.list_type === 'list') {
      yield exitEvent('list', zeroPos(closePt));
    } else {
      yield exitEvent('definition-list', zeroPos(closePt));
    }
  }
}
...
function* parseTable(
  ...
  yield enterEvent('table', attributes !== undefined ? { attributes } : EMPTY_EVENT_PROPS, zeroPos(startPt));
  ...
  while (peek(buf) !== null) {
    ...
    if (t.type === TokenType.TABLE_CAPTION) {
      ...
      yield enterEvent('table-caption', EMPTY_EVENT_PROPS, zeroPos(capPt));
    ...
    if (t.type === TokenType.TABLE_HEADER_CELL) {
      ...
      if (!rowOpen) {
        ...
        // an explicit `|-` line.
        const rowPt = pointAt(buf.tracker, t.start);
        yield enterEvent('table-row', EMPTY_EVENT_PROPS, zeroPos(rowPt));
        rowOpen = true;
      }
      advance(buf);
...
function* parseTableCells(
  ...
  while (true) {
    ...
    yield enterEvent('table-cell', tableCellProps(header), zeroPos(cellPt));
...
function* parseThematicBreak(
  ...
  yield enterEvent('thematic-break', EMPTY_EVENT_PROPS, breakPos);
...
function* parsePreformatted(
  ...
  yield enterEvent('preformatted', EMPTY_EVENT_PROPS, zeroPos(startPt));
